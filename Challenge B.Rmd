...
We take back our previous code to simulate the data and the training and test samples. We will use the function npreg to obtain the local linear models.
...
```
We can see on this final plot that for a very low bandwidth, the best estimation is made on the test data. However, as the bandwidth increases, it is the training data that takes over.
...
```
Step 2 -
We proceed in several steps, as detailed in the code. We obtain the following table, which shows us that the only company left isn't in every single department. 
```{r,echo=FALSE,include=FALSE}
attach(CNIL)
...
#Get the companies who a CNIL in every department (there are 101)
Step2 <-  CNIL2 %>% group_by(SIREN) %>% filter(n()>101) #
Step2
```
...
table(Step2)
```
Therefore, there are no organizations that have nominated a CNIL per department.
Step 3 -
Here comes the funny part. The import takes 5 to 10 minutes here. To gain time, we are only taking the SIREN and the size of the firm from the database. The whole database would take days to get imported. We invite you to run the code manually here, because knitting the whole document with this line in it takes too long.
```{r, echo=FALSE,include=FALSE,eval=FALSE}
"sirendf"<-read.csv(file=file.choose(),
